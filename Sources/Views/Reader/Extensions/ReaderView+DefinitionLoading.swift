...
extension ReaderView {
    ...
    func loadDefinitionData(preservingMeaning: Bool = false) {
        ...
        let request = ReaderDefinitionWorker.Request(sentence: originalSentence, word: selectedTextForDefinition,
            bookId: bookId, mode: translationMode, convertTraditional: shouldConvertTraditionalToSimplified)
        ...
        definitionSession.task = Task { @MainActor in
            do {
                ...
                if definitionSession.requestID == requestID { definitionSession.loading = false }
            }
        }
    }
    ...
    func refreshDefinitionRules() {
        ...
        definitionSession.ruleTask = Task { @MainActor in
            do {
                ...
                guard showingDefinitionSheet, definitionSession.identity == sessionID else { return }
