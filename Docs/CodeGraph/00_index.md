...
* The per-book book row is extracted once into a shared component `BookListItemView` (`Sources/Views/Common/BookListItemView.swift`) — cover via `BookCoverView`, translated title (`TranslateUtils.isTranslationEnabled && containsChinese` → `translateMeta`), Hán-Việt author, and source pill; optional "Đang đọc" chapter line via `showChapter` (default true). `ShelfView.bookItemView` now just returns `BookListItemView(book:)` (behavior unchanged), and `BookShareTargetSheet` reuses it with `showChapter: false`.
