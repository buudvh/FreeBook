...
* Gate: `check_architecture.py` **16 → 14 violation**, tập còn lại là tập con thật sự — mất `LINE_LIMIT_EXCEEDED` của `ShelfView.swift` (1076 → **827**, baseline 942) nhờ tách `+TXTImport` (283) và của `DownloadManager.swift` (688 → **484**, baseline 640) nhờ tách `+TaskStore` (249) + `TxtExportFileWriter` (97). `ReaderView.swift` 2268 → **2186** và `ChapterStoreDatabase.swift` 955 → **954** đều đi đúng chiều; `ChapterPersistenceStore.swift` giữ 915. Không entry allowlist nào được thêm hay nới, không baseline nào bị nới. Tổng file Swift 279 → **284**. Không biên dịch tại chỗ (host Windows, `xcodebuild` chỉ chạy trên macOS) — CI chỉ chứng minh *biên dịch được*.
