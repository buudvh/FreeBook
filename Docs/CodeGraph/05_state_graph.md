...
*   **Trạng thái khả dụng của Remote Command (Derived Availability, Bounded Fallback & Metadata Normalization)**: Trạng thái khả dụng của các lệnh điều khiển từ xa trong `MPRemoteCommandCenter` (`syncRemoteCommandState()`) là các giá trị dẫn xuất (derived properties) dựa trên `active` (`!playingBookId.isEmpty && showFloatingWidget`) và `isPlaying`. Khi phiên active: nếu đang phát (`isPlaying == true`), `pauseCommand.isEnabled = true` và `playCommand.isEnabled = false`; nếu đang tạm dừng (`isPlaying == false`), `playCommand.isEnabled = true` và `pauseCommand.isEnabled = false`. Để tương thích với các thiết bị phụ kiện gửi lệnh `.pause` khi đang tạm dừng, `handleRemoteTransportCommandOnMain(.pause)` đóng vai trò là giải pháp fallback tương thích có phạm vi giới hạn (bounded compatibility fallback), tự động gọi `resume()` khi đang paused. Đồng thời, `updateNowPlayingInfo()` thực hiện chuẩn hóa metadata (loại bỏ `MPNowPlayingInfoPropertyIsLiveStream` và thiết lập `MPNowPlayingInfoPropertyMediaType.audio`) và thống nhất thứ tự ghi `nowPlayingInfo` và `playbackState` để phục vụ thử nghiệm chẩn đoán giao diện Lock Screen.
