...
*   **Trạng thái khả dụng của Remote Command (Derived Availability, Synchronous Dispatch & Metadata Normalization)**: Trạng thái khả dụng của các lệnh điều khiển từ xa trong `MPRemoteCommandCenter` (`syncRemoteCommandState()`) dựa trên `active` (`!playingBookId.isEmpty && showFloatingWidget`). Thử nghiệm kiểm soát Phase E vô hiệu hóa `playCommand.isEnabled = false` và `pauseCommand.isEnabled = false`, đồng thời chỉ bật `togglePlayPauseCommand.isEnabled = active` trong cả `setRemoteCommandsEnabled(_:)` và `syncRemoteCommandState()` nhằm loại bỏ hiện tượng gửi nhầm `action:pause` khi tạm dừng. Để tương thích với các thiết bị phụ kiện gửi lệnh `.pause` khi đang tạm dừng, `handleRemoteTransportCommandOnMain(.pause)` tiếp tục đóng vai trò là giải pháp fallback tương thích có phạm vi giới hạn (bounded compatibility fallback), tự động gọi `resume()` khi đang paused. Đồng thời, khi callback đi vào từ Main Thread (`Thread.isMainThread == true`), `dispatchRemoteTransportCommand` được thực thi đồng bộ via `MainActor.assumeIsolated` trước khi trả về `.success`, giúp đồng bộ trạng thái phát ngay lập tức cho MediaRemote.
