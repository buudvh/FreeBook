...
*   **Trạng thái khả dụng của Remote Command (Derived Availability, Synchronous Dispatch & Metadata Normalization)**: Trạng thái khả dụng của các lệnh điều khiển từ xa trong `MPRemoteCommandCenter` (`syncRemoteCommandState()`) là các giá trị dẫn xuất (derived properties) dựa trên `active` (`!playingBookId.isEmpty && showFloatingWidget`) và `isPlaying`. Khi phiên active: nếu đang phát (`isPlaying == true`), `pauseCommand.isEnabled = true` và `playCommand.isEnabled = false`; nếu đang tạm dừng (`isPlaying == false`), `playCommand.isEnabled = true` và `pauseCommand.isEnabled = false`. Thử nghiệm Phase C vô hiệu hóa `togglePlayPauseCommand.isEnabled = false` trong cả `setRemoteCommandsEnabled(_:)` và `syncRemoteCommandState()` để loại bỏ xung đột lệnh trung tâm trên Lock Screen, tạo cặp lệnh `play/pause` không mơ hồ. Để tương thích với các thiết bị phụ kiện gửi lệnh `.pause` khi đang tạm dừng, `handleRemoteTransportCommandOnMain(.pause)` đóng vai trò là giải pháp fallback tương thích có phạm vi giới hạn (bounded compatibility fallback), tự động gọi `resume()` khi đang paused. Đồng thời, khi callback đi vào từ Main Thread (`Thread.isMainThread == true`), `dispatchRemoteTransportCommand` được thực thi đồng bộ via `MainActor.assumeIsolated` trước khi trả về `.success`, giúp đồng bộ trạng thái phát ngay lập tức cho MediaRemote.
