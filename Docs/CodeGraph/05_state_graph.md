...
*   **Trạng thái khả dụng của Remote Command (Derived Availability, Synchronous Dispatch, Outcome-Aware Status Return, State-Dependent Play/Pause Enablement & Finite Timeline Contract)**: Trạng thái khả dụng của các lệnh điều khiển từ xa trong `MPRemoteCommandCenter` (`syncRemoteCommandState()`) dựa trên `active` (`!playingBookId.isEmpty && showFloatingWidget`), `playing` (`active && isPlaying`) và `paused` (`active && !isPlaying`). Phase J điều chỉnh cờ khả dụng phụ thuộc trạng thái phát: `playCommand.isEnabled = paused`, `pauseCommand.isEnabled = playing`, đồng thời duy trì `togglePlayPauseCommand.isEnabled = active`, `nextTrackCommand.isEnabled = active` và `previousTrackCommand.isEnabled = active`. Thử nghiệm kiểm soát Phase I hoàn thiện hợp đồng metadata Now Playing bằng cách di chuyển `setSystemNowPlayingPlaybackState(_:)` thành instance method private `@MainActor` trong `TTSManager`, xuất bản đồng bộ bộ 5 key timeline tiêu chuẩn (`currentRate: 1.0/0.0`, `defaultRate: speed`, `elapsed`, `duration`, `progress`) và đồng bộ trong `updateNowPlayingInfo()` để tránh bị ghi đè, hỗ trợ MediaRemote hiển thị timeline hữu hạn và đồng bộ biểu tượng Lock Screen.
