...
* `ExtensionScriptEditorView` thêm state trình bày `showingDebugRun` và `debugRunEntrypoint`. Nó không giữ runner hay event buffer; khi mở sheet, `ExtensionDebugConsoleView` tự sở hữu `ExtensionDebugTraceReader` như màn debug cũ.
* `ExtensionDebugConsoleView` thêm state một lượt `didApplyInitialSelection` để seed `selectedPackageId` và input entrypoint đúng một lần. Sau khi người dùng đổi picker/input trong màn debug, init value không ghi đè lại lựa chọn của họ.
* `isAutoScrollDisabled` có thêm một nguồn ghi `false`: `reenableTTSAutoScrollFromWidgetJump`, chỉ chạy khi nhận `navigateReaderToPlayingChapter` đúng sách từ widget. Ba nguồn tắt (`header`, tìm kiếm, kéo tay) vẫn giữ nguyên; lệnh widget là ý định rõ ràng quay về highlight TTS nên được phép bật lại.
...
* **`selectedWordOffset` đổi trong lúc panel mở không còn tự chạy `refreshRuleTraces()`**. State rule trace bám theo `originalSentence` (đoạn gốc) và signal rule, không bám theo từng token offset; vì vậy nới/thu vùng chọn chỉ cập nhật nghĩa/selection rẻ, còn đổi đoạn hoặc đổi rule mới chẩn đoán lại. `refreshRuleTraces` vẫn giữ focus nếu chip cũ còn tồn tại, chỉ xoá `focusedRuleTraceID` khi rule đó biến mất khỏi kết quả.
...
* **`revision` của hai store mới không phải state của Reader.** Nó là `@MainActor @Published` chỉ để đẩy UI danh sách/hub; nó **không** đi vào cache key dịch — đường invalidation rule là `notifyRulesDidUpdate`, còn từ điển vẫn là `notifyDictionariesDidUpdate`.
...
* `isAutoScrollDisabled` nay có bốn nguồn ghi ngoài giá trị bootstrap: nút bật/tắt ở header, `jumpToReaderSearchResult` (ghi `true`), `handleUserScrollWhilePlaying` (ghi `true`), và `reenableTTSAutoScrollFromWidgetJump` (ghi `false` khi widget yêu cầu nhảy về highlight). Cờ vẫn **không được persist**: `ReaderView.swift` đọc `UserDefaults("disableAutoScroll_\(bookId)")` một lần lúc dựng và không nơi nào ghi key đó, nên trạng thái tắt là **phạm vi phiên**.
...
* Cổng render của `singleChapterReaderView` đọc đúng trạng thái đó: nhánh mới `pending != displayedChapterIndex` → `chapterInlineLoadingView(index: pending)` đặt **trước** nhánh `cache.get(presentationIndex)?.state == .loaded`. Vì `ChapterCache` trước 1.3.375 không evict (`queueRelease*` không có caller) và N+1 được prefetch, chương đích gần như luôn `.loaded`; không có nhánh này thì skeleton không bao giờ được vẽ khi Next/Prev. **Từ 1.3.375 `queueReleaseAllNonVisible` được gọi thật** — sau mỗi commit navigation, `ReaderView.applyNavigationCommit` giữ cửa sổ ±3 quanh chương vừa tới và `queueRelease` ân hạn 5 s (huỷ hẹn nếu `get()` chạm lại chương đó). Nên tiền đề "không bao giờ evict" **không còn đúng**: chương ngoài cửa sổ có thể bị gỡ. Cổng render vẫn đúng vì hai nhánh cùng bao hết trường hợp cache miss (miss ⇒ `chapterInlineLoadingView`, rồi `requestChapter` nạp lại). Reload cùng chương (`pending == displayedChapterIndex`) vẫn đi nhánh cũ nên không nháy skeleton.
