...
stateDiagram-v2
    ...
    Stopped --> Preparing : startSpeaking() / Cache prewarm chưa có
    Stopped --> Playing : startSpeaking() / Dùng kết quả prewarm
...
*   **Prewarm không đổi trạng thái phát**: `prepareSpeaking(...)` chỉ chuẩn hóa/phân đoạn chương Reader vào cache riêng; không đổi chương hoặc trạng thái của phiên TTS.
*   **`Stopped` -> `Preparing` / `Playing`**: `startSpeaking(...)` phát ngay nếu cache prewarm khớp; nếu cache miss thì dùng processor riêng ưu tiên cao rồi bắt đầu phát.
...
*   **Trạng thái khả dụng của Remote Command (Derived Availability, Synchronous Dispatch, Outcome-Aware Status Return & Metadata Normalization)**: Trạng thái khả dụng của các lệnh điều khiển từ xa trong `MPRemoteCommandCenter` (`syncRemoteCommandState()`) dựa trên `active` (`!playingBookId.isEmpty && showFloatingWidget`). Thử nghiệm kiểm soát Phase E giữ `playCommand.isEnabled = false` và `pauseCommand.isEnabled = false`, đồng thời chỉ bật `togglePlayPauseCommand.isEnabled = active`. Thử nghiệm kiểm soát Phase F cập nhật `dispatchRemoteTransportCommand` tính toán giá trị `MPRemoteCommandHandlerStatus` phản hồi dựa trên trạng thái `isPlaying` thực tế sau khi thực thi (`.success` nếu trạng thái cuối khớp yêu cầu, `.commandFailed` nếu mâu thuẫn như khi kích hoạt fallback `resume()` cho lệnh `.pause` lúc paused). Thử nghiệm kiểm soát Phase G chuẩn hóa xuất bản metadata qua helper `setSystemNowPlayingPlaybackState(_:defaultPlaybackRate:)` và `updateNowPlayingInfo()` với tốc độ tương đối `MPNowPlayingInfoPropertyPlaybackRate` (`1.0` khi playing / `0.0` khi paused) và `MPNowPlayingInfoPropertyDefaultPlaybackRate` mang tốc độ đọc TTS (`speed`), hỗ trợ MediaRemote nhận diện đúng biểu tượng hiển thị (cần kiểm chứng thực tế trên thiết bị). Đồng thời, khi callback đi vào từ Main Thread (`Thread.isMainThread == true`), `dispatchRemoteTransportCommand` được thực thi đồng bộ via `MainActor.assumeIsolated` trước khi trả về status cho MediaRemote.
...
*   Reader Next/Previous/Chapter List không tạo transition cho TTS; chương TTS chỉ đổi khi người dùng chủ động Start hoặc khi TTS tự đọc hết chương.
