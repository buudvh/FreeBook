...
*   **Trạng thái khả dụng của Remote Command (Derived Availability, Synchronous Dispatch, Outcome-Aware Status Return, Metadata Normalization & Triple-Command Enablement)**: Trạng thái khả dụng của các lệnh điều khiển từ xa trong `MPRemoteCommandCenter` (`syncRemoteCommandState()`) dựa trên `active` (`!playingBookId.isEmpty && showFloatingWidget`). Thử nghiệm kiểm soát Phase H bật khả dụng đồng thời cả 3 lệnh `playCommand`, `pauseCommand` và `togglePlayPauseCommand` (`isEnabled = active/enabled`), phối hợp với bộ xuất bản metadata tương thích của Phase G (`setSystemNowPlayingPlaybackState(_:defaultPlaybackRate:)` và `updateNowPlayingInfo()` với `currentRate: 1.0/0.0`, `defaultRate: speed`), phản hồi trạng thái Phase F và điều phối đồng bộ MainActor Phase B để kiểm chứng khả năng cập nhật biểu tượng trực quan Lock Screen (cần kiểm chứng thực tế trên thiết bị).
