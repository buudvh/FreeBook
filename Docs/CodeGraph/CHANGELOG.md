...
* **`ReaderChapterListStore` dùng một nguồn sự thật duy nhất `rows: [Int: ReaderChapterRowState]`** (thay cho `loadedRowStates` + `pageCache` + `pageRequestIDs` + `latestWindowRequestID` + `deferredPrefetchTask` + `pageRetryTask` + `activeLoadingTargetPage` + `currentTargetPage` + `hasLoadedRows` + `prunePageCache` + `publishCachedPageIfAvailable` + `reloadViewportAfterReset`). Trạng thái hàng thay vì "cửa sổ 3 trang": trang nào được nạp thì lưu `rows` (mỗi trang 100 hàng), placeholder "Đang tải..." dựng theo yêu cầu. Memory tỷ lệ với số trang thực sự lướt qua, không phải tổng số chương.
* **Một đường load duy nhất `loadPagesAround(pages:targetPage:)`**: các wrapper `loadPageIfNeeded`, `loadVisiblePageIfNeeded`, `prefetchAround`, `jumpToChapter` đều đi qua đó. Dedup tải bằng `inFlightPages` (dùng chung Task cho cùng trang), publish **từng trang độc lập** ngay khi xong, và **retry một lần sau 300ms** cho trang thất bại (giữ lại Fix A chống skeleton từ [1.3.203]). `watcher` gom `failedPages` từ các task đã hoàn thành rồi `scheduleRetry`.
* **Guard idempotent**: `updateChapters`/`updateSortOrder`/`updateTranslation` early-return khi không có gì đổi (`totalCount` + `onlineChapters` so sánh `Equatable`; sort; translation flag). Khi có đổi thì `reset()` (tăng generation, cancel task đang bay + retry, xoá rows + trạng thái search) rồi `reloadViewport()` — chỉ reload trang đang xem (`lastRequestedPage`), không còn fallback về trang 0.
* **Xoá `ReaderChapterListPageFetcher`** (`Sources/Services/ChapterText/Workers/`): việc fetch gộp vào `ReaderChapterListStore.fetchPageData`, giữ nhánh `modelContext == nil` (sách online) nạp thẳng từ `onlineChapters` và **bỏ qua hàng có URL rỗng** (thay vì trả về chuỗi có placeholder); nhánh SwiftData vẫn dùng `BackgroundPagingWorker`. Bỏ luôn fallback `onlineChapters` trong worker (Fix C [1.3.203] tiếp tục đúng) và toàn bộ cơ chế `pageCache`/prefetch xa.
* **`prefetchAround` chỉ làm ấm trang lân cận ±1** (bỏ prefetch xa ±15 dòng), cắt giảm đáng kể số lần fetch dư thừa khi chỉ đọc quanh vị trí hiện tại.
* **P1.1 – bỏ cập nhật trùng lặp `updateChapters(totalCount:onlineChapters:[])` trong `ReaderView.onLocalTOCRefreshed`**: trước đây nó ghi đè `onlineChapters = []` sau khi `ReaderChapterListView+Refresh.refreshChapters` đã đẩy đúng list từ nhánh sách online, khiến danh sách chương của sách online bị mất dữ liệu khi làm mới mục lục. `refreshChapters` đã luôn gọi `store.updateChapters` ở cả 3 nhánh trước khi fire `onLocalTOCRefreshed` nên việc gọi lại chỉ gây hại.
* **Xoá test cũ phụ thuộc API đã gỡ**: `ReaderViewModelTests.swift` bỏ `testBoundedMemoryFor20kChapters`, `testRepeatedSamePageRequestsFetchOnce`, `testUpdateChaptersReloadsLastViewport`, `testMoveCenterPageRetainsOldUntilAtomicSwap`, `testStaleGenerationCannotPublish`, `testFailureClearsInFlightAndRetrySucceeds`, `testRowStateReadDoesNotMutateAndItemBounds`, `testPrefetchWarmsCacheAndDoesNotPublish`, `testVisiblePageUsesPrefetchedCacheWithoutPlaceholderGap`, `testSameGenerationOlderWindowStalePrevention`, `testPageCacheEvictionAndBounding`, `testNormalLoadAndPrefetchDoesNotToggleLoading`, `testJumpToChapterLoadsCurrentPageBeforeDeferredNeighbors`, `testSortChangeResetsCoordinatorAndCanReloadSamePage` và `TestProbe`. Giữ nguyên `testChapterListStoreMarksOnlyRequestedRowCached`, `testZAMapping`, `testSearchLimit` (đều dùng API còn sống) và `testCollisionSafePersistenceWithRealAPI` (thuộc `ChapterPersistenceStore`).
* **Lưu ý hạ tầng**: file `ReaderChapterListPageFetcher.swift` bị xoá khỏi cả `manifest.json` (16 documents), `Docs/CodeGraph` cập nhật vùng `<!-- GENERATED -->` của `00_index.md`, `02_file_graph.md`, `04_call_graph.md`, `11_subsystems.md`, `13_resource_lifecycle.md`. `sourceFileCount` 215 -> 214. Validator PASS (read-only + `--update-hashes`). Build/test chỉ kiểm chứng được trên macOS/CI — repo đang mở trên Windows.
