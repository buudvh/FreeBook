...
Thêm **25** file Swift (423 → 448), sửa 8 file; **không** `@Model` nào đổi shape, **không** thêm dependency. Chưa biên dịch (viết trên Windows, không có `xcodebuild`/`xcodegen`) — **phải chạy `xcodegen generate` trên macOS/CI** vì có file mới.

* **`tool == "nghitts"` nay nghĩa "engine chạy trên máy", không còn nghĩa "Piper".** Engine cụ thể do khoá UserDefaults `nghiEngineKind` ∈ `{piper, vieneu}` chọn, mặc định `piper`. Giữ nguyên chuỗi `"nghitts"` là chủ ý chứ không phải tiện tay: nó được so sánh ở **~45 chỗ** trong `TTSManager` cộng `TTSChapterPrefetcher`, `TTSNextChapterPrefixCache`, `TTSManager+PrefetchCache`, `TTSManager+NextChapterPrefix`, và là proxy điều khiển toàn bộ policy on-device (watermark prefetch, trim cache, next-chapter prefix, energy log) cùng phép phân loại remote `tool != "system" && tool != "nghitts"`. Thêm một `tool` value thứ năm buộc phải rà lại cả 45 nhánh, và bỏ sót một chỗ là VieNeu bị xếp vào nhóm remote.
* **Điểm nối là protocol mới `LocalTTSSynthesizing`** — đúng 5 thành viên mà tầng trên thật sự gọi. `PiperTTSService` chỉ thêm conformance, **không đổi thân hàm nào**; `TTSChapterPrefetcher`/`TTSNextChapterPrefixCache`/`TTSManager` đổi từ kiểu cụ thể `PiperTTSService` sang `any LocalTTSSynthesizing` (đây cũng là chỗ phụ thuộc sai chiều nay đã bỏ). Giá trị mặc định của tham số **phải** khai ở extension của protocol: mặc định khai trong kiểu cụ thể không đi qua witness table nên call site bỏ bớt `requestID` sẽ vỡ. Cả hai engine dùng chung `PiperSynthesisCoordinator` để bất biến "chỉ một tác vụ tổng hợp tại một thời điểm" vẫn đúng.
* **Phân hệ mới `Services/TTS/VieNeuTTS/` (21 file)**: model store + downloader (resume bằng HTTP Range, nguyên tử ở mức cả bộ qua staging), đọc npz qua central directory với fp16→fp32 bằng `vImageConvert_Planar16FtoPlanarF`, `SeaG2PDictionary` mmap `sea_g2p.bin` và **so sánh theo byte UTF-8** thay vì dựng `String` mỗi lần probe (bỏ ~17 lần cấp phát mỗi lượt tra, đồng thời sửa lỗi tiềm ẩn: `String` của Swift so sánh sau chuẩn hoá Unicode nên thứ tự có thể lệch khỏi thứ tự byte mà bảng được Rust sắp, làm tìm kiếm nhị phân **trượt** một số từ mà không báo lỗi), tokenizer BPE byte-level, và vòng suy luận.
* **Vòng suy luận tốn `1 + n_vq = 17` lời gọi ONNX cho mỗi 80 ms audio** (~212 lời gọi mỗi giây), tuần tự tuyệt đối. KV cache truyền thẳng `ORTValue` output → input bước sau, **không** qua `tensorData()`: ở `T = 300` một lượt copy cả cache là ~12 MB. Lấy mẫu chọn top-k **trước** rồi mới softmax/nucleus trên 25 ứng viên, thay vì cấp phát 1024 tuple rồi sort toàn bộ 16 lần mỗi frame. `VieNeuTensor` tồn tại vì `ORTValue(tensorData:)` **bọc con trỏ** chứ không giữ tham chiếu `NSMutableData`.
* **Dùng bộ `onnx_int8`, không phải `onnx` fp32.** Số đo trên iPhone 11 cho decode step 19.92 ms/frame khi đọc 415 MB weight, tức ≈ **20.8 GB/s** — thuần bị chặn băng thông, nên bản int8 (104 MB) giảm gần 4× ở hạng mục chiếm 64% vòng lặp. Kèm theo là **bốn** khác biệt kiến trúc, không chỉ đổi URL: 1 local layer thay vì 2 (nên tên input/output được **phát hiện từ session** qua `VieNeuSessionLayout`, và danh sách KV sắp theo **số** ở đuôi tên chứ không theo thứ tự băm của `outputNames()`), cần speaker embedding (x-vector 192 chiều qua `xvec_*` + LayerNorm phương sai toàn phần, cộng vào **mọi** hàng prompt), heads fp32, và token dẫn đầu prompt phải là **16**.
* **Token dẫn đầu 16, không bao giờ 17/18.** `config.json` khai `reserved_token_start = 13`, `num_reserved_tokens = 30` nên id 13..42 là ô dự trữ **random-init chưa train** — trong đó có cả `tin_tuc` (17) và `doc_truyen` (18). Nhãn phong cách trong `voices_v3_turbo.json` **chỉ để hiển thị**.
* **Giữ đủ 16 codebook.** Codec MOSS là RVQ **residual**: nhồi code 0 vào các tầng cuối không phải im lặng mà là cộng thêm một vector residual cụ thể, làm tiếng đục và rè. Giảm bitrate đúng cách là đưa decoder ít codebook hơn.
* **Ba lá chắn chống ảo giác của model tự hồi quy**: trần frame `24 + 2.0 × số ký tự phoneme` (chặn trên 300) thay trần cố định, repetition penalty **cửa sổ trượt 64 frame** cho **mọi** codebook (`Set` tích luỹ sau vài trăm frame đã phạt gần nửa codebook kể cả code phải lặp lại, làm giọng trôi về cuối chunk), và chốt dấu câu cuối chunk theo luật `punc_norm`.
* **Hồ sơ tiền xử lý `TTSPreprocessProfile`**: cờ `transliterationEnabled` cũ tách thành kana→Romaji (cả hai engine cần — sea-g2p **không có** tiếng Nhật) và Latin→âm Việt (chỉ Piper cần — VieNeu đọc tiếng Anh gốc, phiên âm trước là phá). Chuẩn hoá số/ngày/giờ/tiền và từ điển người dùng áp cho **cả hai**; với VieNeu còn bắt buộc hơn vì bộ ký hiệu phoneme của sea-g2p dùng **chữ số làm dấu thanh** (`aː2`, `a6j`, `iɛ6n`) nên một chữ số thô lọt xuống G2P sẽ bị đọc thành thanh điệu. Khoá cache phiên âm mang cả hồ sơ để hai hồ sơ không trả kết quả của nhau. Thứ tự cũ "cả token là Romaji Nhật thì xử lý nguyên khối **trước** khi xét gạch nối" được giữ nguyên, đảo thứ tự sẽ làm "a-ri-ga-to" đọc khác đi.
* **Giọng hai engine lưu ở hai khoá** (`nghittsVoice` và `vieneuVoice`) vì hai bộ tên không giao nhau. `selectedVoice` vẫn đi theo đường cũ vì nó tham gia khoá tổng hợp và phép kiểm tra danh tính ở hai call site tổng hợp, nên `VieNeuTTSService` **tự phân giải lại** tên giọng — đổi đường truyền là chạm vào logic huỷ/hợp lệ của cả hàng chờ. Đổi engine giữa phiên `stop()` + `clearPrefetchCache()` vì hai engine cho 22.05 kHz và 48 kHz; WAV im lặng đường VieNeu cũng phải 48 kHz. Chọn VieNeu mà chưa tải model thì `nghiTTSService` là `nil` và người dùng thấy lỗi tường minh, **không** im lặng rơi về Piper.
* **Model tải runtime ~274 MB, 11 file trong một thư mục, không được đổi tên** — hai graph tham chiếu file weight bằng **tên trần** trong external-data proto và ORT phân giải tên đó tương đối với thư mục chứa `.onnx`. Mỗi file có ngưỡng cỡ tối thiểu để loại trang lỗi HTTP 200, thứ mà `URLSession` không coi là thất bại nhưng sẽ làm `ORTSession` chết bằng thông báo không liên quan. Không dùng `URLSession.bytes(for:)`: nó là `AsyncSequence` của **từng byte**, 274 MB thành 274 triệu lần `await next()`.
* **Ba file sát/vượt trần đều không phình**: `TTSManager.swift` giữ **đúng 4000** (một dòng thay thế; logic mới ở `TTSManager+LocalEngine`, và `nghiEngineKind`/`vieNeuVoiceName` là **computed property đọc/ghi UserDefaults** thay vì `@Published` chính vì lý do này — setter tự gọi `objectWillChange.send()`, View phải dùng `Binding(get:set:)`), `TextPreprocessor.swift` **giảm 1121 → 1120**, `TTSSettingsView.swift` **giảm 519 → 515**. `TTSModelManagerView.swift` giữ nguyên 478 — màn tải VieNeu là file riêng treo từ `TTSSettingsSection`.
* `check_architecture.py` giữ **14 violation, đúng cùng một tập** trước/sau; 25 file mới đều ≤ 400 dòng và đúng 1 type top level, không nới baseline, không sửa `architecture_allowlist.json`. CodeGraph: cập nhật `00`, `02`, `03`, `04`, `05`, `09`, `10`, `11`, `13`, `14`; `06`, `08`, `rules` ghi nhận `--no-change-needed`.
* **Ba điểm phải kiểm ngay ở lần build/chạy đầu**, xem `10_risk_report.md`: selector `ORTSessionOptions.addConfigEntry(withKey:value:)` có trong binding ObjC đang pin hay không (thiếu là **lỗi biên dịch**), ORT có mmap chung `vieneu_backbone_shared.data` cho hai session hay không (nếu copy thì RAM 208 MB thay vì 104 MB), và bản SPM có contrib op int8 hay không (thiếu thì đường lùi là bộ `onnx` fp32).
