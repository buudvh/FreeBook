...
* **Định danh hàng rule là `rowID` (UUID của snapshot), không phải `sourceLine`** — số dòng đổi sau mỗi lần thêm/xoá (nguyên nhân crash đã sửa ở 1.3.271). `QuickTranslationRuleTrace.id` là `"\(rowID)#\(sourceRange.location)"` — xác định, không phải `UUID()` mới mỗi lượt; đổi rule nguồn khi truyện bị đẩy khỏi LRU cap 3 của `QuickTranslationRuleBookStore` thì `deleteRule(rowID:)` từ chối an toàn thay vì xoá nhầm.
