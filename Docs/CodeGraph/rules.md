...
* `ChapterContentRepository` is the only chapter-content loader: memory -> ChapterSQLiteRepository -> extension. Non-empty normalized content read via `BookBinManager` based on `ChapterModel.offset`, `ChapterModel.length`, and `ChapterModel.isCached` is authoritative. Chapter metadata is stored in a native SQLite3 table (`chapters`) using `actor ChapterSQLiteRepository` and accessed via `ChapterRepositoryProtocol`, completely decoupled from SwiftData.
