...
8. **Phân hệ rule dịch Quick Translate (1.3.269)** — `Sources/Services/Translation/Engine/QuickTranslation*.swift` + `Sources/Views/Settings/Translation/QuickTranslation*.swift`:
   ...
   - Chủ sở hữu duy nhất của bộ rule là `QuickTranslationRuleStore`. Bộ rule **không** đi kèm app: nó là file `translate/QuickTranslateRules.txt` với đúng hai đường vào — nút tải về từ HuggingFace (cùng dataset với VietPhrase/PhienAm) hoặc người dùng nhập file. Mọi đường đều **validate-then-swap** (một hard error là không ghi file, không swap, bộ đang chạy giữ nguyên) và phát `generation` đi vào cache dịch.
