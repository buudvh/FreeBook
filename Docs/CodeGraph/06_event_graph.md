...
<!-- GENERATED START -->
## TTS floating widget window events (1.3.193)

* The floating widget no longer lives in `AppLaunchRootView`'s ZStack; it is hosted by `TTSFloatingWidgetWindowManager.shared` in a dedicated `UIWindow` (`windowLevel = .alert`), so it stays above every modal presentation (Reader `fullScreenCover`, bypass-browser `pageSheet`/cover, sheets). The window is non-key and its `hitTest` returns `nil` outside the widget's current frame — `TTSFloatingWidgetView` reports the frame through `.onAppear`/`.onChange(of: widgetFrame)`, which updates on drag/peek/reveal.
* Show/hide is driven by `AppLaunchRootView.refreshTTSWidgetWindow()` (onAppear + `.onChange` of `translationManager.isInitialized`, `ttsPresentation.snapshot.showFloatingWidget`, and `ttsPresentation.snapshot.showingSettingsSheet`): it calls `presentWidget()` only when `isInitialized && showFloatingWidget && !showingSettingsSheet`, else `dismissWidget()`. `TTSManager.shared.showingSettingsSheet` is the single gate — the widget hides while `TTSSettingsSheet` is open and reappears on dismissal.
* Widget actions are unchanged: cover button posts `openCurrentlyPlayingReader` (MainTabView switches to `selectedTab = 0`, ShelfView sets `readerPresentationRoute` → cover presents); gear sets `showingSettingsSheet = true`; `stop()` clears `showFloatingWidget` → `refreshTTSWidgetWindow()` dismisses the window.
* `TTSFloatingWidgetWindowManager` re-attaches on `UIApplication.didBecomeActiveNotification` when the active `UIWindowScene` changed (recreates the window on the current scene).
