...
*   `playCommand` -> Kích hoạt khi active (`active`), gọi `TTSManager.resume()`. Giữ `isEnabled = active` đồng thời với `pauseCommand` (Phase D) để tránh hiện tượng nút Play bị mờ/xám sau khi pause.
*   `pauseCommand` -> Kích hoạt khi active (`active`), gọi `TTSManager.pause()`. Giữ `isEnabled = active` đồng thời với `playCommand` (Phase D). Nếu nhận được sự kiện remote `.pause` khi phiên đang ở trạng thái paused, `handleRemoteTransportCommandOnMain(.pause)` kích hoạt cơ chế tương thích có phạm vi giới hạn (bounded compatibility fallback) gọi `resume()`. Đồng thời, khi callback đi vào từ Main Thread (`Thread.isMainThread == true`), `dispatchRemoteTransportCommand` được thực thi đồng bộ via `MainActor.assumeIsolated` trước khi trả về `.success`, giúp MediaRemote đọc được `playbackState` mới ngay lập tức để đồng bộ biểu tượng nút Play/Pause trên Lock Screen.
*   `togglePlayPauseCommand` -> Đăng ký target handler trong `setupRemoteCommandCenter()`, nhưng bị vô hiệu hóa (`isEnabled = false`) trong `setRemoteCommandsEnabled(_:)` và `syncRemoteCommandState()` (thử nghiệm Phase C) để tránh xung đột nút điều khiển trung tâm trên Lock Screen UI.
