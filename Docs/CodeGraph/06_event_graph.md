...
*   `playCommand` / `resumeCommand` -> Cập nhật đồng bộ `playbackState = .playing` và kích hoạt `TTSManager.resume()`, sử dụng `lastRemoteCommandTime` để chống trùng lặp sự kiện.
*   `pauseCommand` -> Cập nhật đồng bộ `playbackState = .paused` và kích hoạt `TTSManager.pause()`, sử dụng `lastRemoteCommandTime` để chống trùng lặp sự kiện.
*   `togglePlayPauseCommand` -> Được kích hoạt để xử lý trực tiếp sự kiện nhấn nút trên tai nghe, đồng thời sử dụng `lastRemoteCommandTime` để chống trùng lặp sự kiện (de-duplication) khi nhận song song các lệnh ánh xạ khác.
