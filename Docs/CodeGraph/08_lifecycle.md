...
## Vòng đời panel Dịch và refresh chương latest-wins (1.3.362)

* Mở panel tạo identity mới và hiện UI ngay; worker nền tải token/meaning/matches/suggestions theo `SelectionSnapshot`, còn trace debounce 150 ms sau khi dữ liệu definition hiện tại được publish. Đổi selection tạo request mới nhưng giữ dữ liệu cũ trên màn cho tới khi kết quả mới thắng.
...
* **Sửa số dòng của mục 1.3.334 ngay dưới**: mốc đóng panel Dịch nay ở `ReaderView.swift` (`.onChange(of: showingDefinitionSheet)` → `handleDefinitionPanelClosed()`). Mốc đổi selection không còn là `.onChange(selectedWordOffset)` riêng; nó đi qua `updateEditorFromSelection()` → `loadDefinitionData()` → `refreshRuleTraces()` sau khi snapshot definition thắng. Phát biểu "hậu xử lý không nằm trong `closeDefinitionPanel()`" **vẫn đúng**.
...
* **Mốc mở là `openDefinitionPanel()`, và thứ tự trong nó là bất biến**: `definitionSession.begin()` → đóng panel khác → reset `didChangeRuleData` + `focusedRuleTraceID` + `ruleTraces` → `showingDefinitionSheet = true` → defer translation refresh → `updateEditorFromSelection()`. Panel mở ngay; dữ liệu definition/rule điền vào sau theo latest-wins thay vì chặn cú mở.
...
* **Cùng `.onChange` đó chỉ nạp lại `searchEngines` khi panel mở**; rule trace nay do request definition hiện tại gọi sau khi publish. `refreshRuleTraces()` giữ nguyên chip đang chọn nếu nó còn tồn tại sau lượt dò; chip mất thì focus rơi về `ruleTraces.first`.
...
* **Vòng đời một lần đổi bộ rule là canonical-write-then-swap, không có trạng thái nửa vời**: parse records → bỏ dòng hỏng và duplicate sau dòng đầu → compile phần hợp lệ vào staging → `Data.write(to:options:.atomic)` rồi swap snapshot dưới `NSLock`, `generation += 1`, dọn cache, phát notification.
* **Snapshot không còn định danh hàng riêng**: `sourceLine` vẫn là toạ độ vật lý/tiebreak của file canonical, còn thao tác UI dùng `pattern` vì record store bảo đảm duy nhất trong file đã ghi.
* **Xoá hàng là giao dịch key-based**: mutation lock nối tiếp đọc records → bỏ record có pattern được chọn → compile/ghi canonical → swap. Nếu key không còn trong file hiện tại thì trả lỗi trước khi ghi/swap.
