...
<!-- GENERATED START -->
## Số liệu sau khi thêm tìm toàn văn offline (1.3.257)

* Tổng file Swift 334 → **344** (+10, phân hệ mới `Sources/Services/Search/` 7 file): [`ChapterSearchIndexDatabase.swift`](../../Sources/Services/Search/ChapterSearchIndexDatabase.swift) **397**, [`ChapterSearchIndex.swift`](../../Sources/Services/Search/ChapterSearchIndex.swift) **189**, [`ChapterSearchIndexBuilder.swift`](../../Sources/Services/Search/ChapterSearchIndexBuilder.swift) **151**, [`ShelfContentSearchView.swift`](../../Sources/Views/Shelf/ShelfMain/ShelfContentSearchView.swift) **123**, [`ChapterSearchIndexSettingsView.swift`](../../Sources/Views/Settings/Search/ChapterSearchIndexSettingsView.swift) **120**, [`ChapterSearchIndexPath.swift`](../../Sources/Services/Search/ChapterSearchIndexPath.swift) **83**, [`ChapterSearchPolicy.swift`](../../Sources/Services/Search/ChapterSearchPolicy.swift) **48**, [`ChapterSearchSnippetBuilder.swift`](../../Sources/Services/Search/ChapterSearchSnippetBuilder.swift) **45**, [`ChapterSearchHit.swift`](../../Sources/Services/Search/ChapterSearchHit.swift) **18**, [`ChapterSearchSettingsSection.swift`](../../Sources/Views/Settings/Main/ChapterSearchSettingsSection.swift) **12**. Tất cả dưới trần 400 và đúng 1 type top level (`Row` nest trong `ChapterSearchIndexDatabase`, `IndexError` nest trong `ChapterSearchIndexPath`, `Statistics` nest trong `ChapterSearchIndex`, `Target`/`Summary` nest trong `ChapterSearchIndexBuilder`, `SearchScope` nest trong `ShelfSearchView`) ⇒ **không** entry `architecture_allowlist.json` nào được thêm; `check_architecture.py` vẫn đúng **14 violation** với tập y hệt.
* `ChapterSearchIndexDatabase.swift` **397/400** là file mới sát trần nhất của repo. Nó từng 405 dòng; cách hạ không phải xoá comment mà gộp hai khối kiểm PRAGMA thành một helper `requirePragma(_:expected:errorCode:)`. Thêm bất kỳ câu SQL mới vào đây là **vượt trần** — statement mới phải đi kèm một lần tách file, không phải một entry allowlist.
* Điểm nóng của đợt này là **dung lượng đĩa, không phải CPU**: tokenizer `trigram` sinh một token mỗi cửa sổ 3 ký tự **và** lưu lại nguyên nội dung để dựng snippet, nên chỉ mục lớn hơn text vài lần. Đó là lý do tính năng mặc định tắt, có màn hiển thị `ByteCountFormatter` của chính file sqlite (`totalByteSize` cộng cả `-wal`/`-shm`) và có nút xoá chỉ mục kèm `VACUUM` ngay — người dùng tắt tính năng là để lấy lại chỗ.
* Chi phí xây lại tỉ lệ với số chương **đã cache**, và nó đọc lại từng chương từ `.bin`: builder nhường CPU bằng `Task.sleep(1 ms)` mỗi [`builderYieldInterval = 50`](../../Sources/Services/Search/ChapterSearchPolicy.swift#L28) chương và huỷ được giữa đường. Vòng đếm và vòng ghi là **hai lượt** qua `fetchOrderedTOC` — cố ý, để thanh tiến độ có tổng đúng ngay từ đầu thay vì nhảy giật.
* Chi phí mỗi lần **tìm** có hai phần tách rời: SQL chỉ `ORDER BY rank LIMIT 60`, còn định vị đoạn nằm ở [`ChapterSearchSnippetBuilder`](../../Sources/Services/Search/ChapterSearchSnippetBuilder.swift#L14) — nó chạy `ChapterTextNormalizer.normalize` **trên từng hàng trả về**, tức tối đa 60 lần chuẩn hoá mỗi truy vấn. Vì vậy có debounce 300 ms ở [`ShelfContentSearchView`](../../Sources/Views/Shelf/ShelfMain/ShelfContentSearchView.swift#L17) và chặn cứng truy vấn < 3 ký tự; bỏ một trong hai là gõ tới đâu chuẩn hoá tới đó.
* Rủi ro tồn đọng đã biết: chỉ mục chỉ phủ chương **đã tải về**, và số chương trong chỉ mục có thể lệch mục lục sau khi nguồn đánh số lại — đường đọc tự hoà giải bằng `fetchOrderedTOC` (còn url ⇒ sửa số, mất url ⇒ bỏ hit) nhưng **không** tự sửa lại đĩa, nên hàng cũ chỉ biến mất khi chương đó được cache lại hoặc người dùng xây lại chỉ mục.
