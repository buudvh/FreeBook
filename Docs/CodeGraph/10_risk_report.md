...
* **Vì sao "sau vài commit hôm qua"**: bốn chi phí trên có từ trước, nhưng 1.3.334 đưa công cụ sửa rule vào panel Dịch **trong Reader**, nên từ đó mỗi lần tinh chỉnh rule/VP đều kéo theo một lượt dựng lại cả chương ngay tại chỗ đang đọc. Tần suất đổi, không phải đơn giá.
* **Rủi ro còn mở: vẫn dựng lại *cả chương* cho một từ.** Lượt này chỉ bỏ phần việc lặp; không có đường dịch tăng dần theo dòng bị ảnh hưởng. Nếu người dùng vẫn thấy nóng sau 1.3.339 thì bước tiếp theo là dịch lại theo dòng, và đó là đổi kiến trúc, phải làm ở lượt riêng.
* **Rủi ro mới của memo tokenize**: `TokenizeMemo` không có đường `clear()` nào được gọi tự động. Nó đúng **chỉ khi** mọi thay đổi ảnh hưởng tới tokenize đều bump `translationGenerationToken` (globalGeneration / bookGenerations / settingsGeneration / `QuickTranslationRuleStore.cacheTag`). Thêm một nguồn dữ liệu mới cho tokenizer mà không bump generation ⇒ memo trả kết quả cũ **im lặng**. Hai cờ đại từ / luật nhân hoá đã được đưa vào khoá tường minh vì chúng đọc thẳng `UserDefaults`.
* **`unusedCapture` hạ xuống `warning` là nới một cổng kiểm tra.** Rule gõ nhầm `{1}` (định trỏ `<L>` mà viết sai chỉ số) trước đây bị chặn cả dòng, nay sẽ chạy và cho bản dịch **thiếu chữ** — im lặng ở mức dịch, chỉ còn cảnh báo trong danh sách lỗi của màn quản lý. Đổi lại `第<n><L> = Chương {0}` mới dùng được. Số hard error của `rule-aio.txt` vì thế còn **1** (dòng 916 `UNBALANCED_PARENS`), không còn 2.
* **`TextDictionary` đổi cách đếm độ dài khoá từ `Character` sang UTF-16.** Đây là **sửa lệch có thật**: hai hàm tra làm việc trên `[UInt16]` nhưng `maxWordLength` cũ đếm `Character`, nên khoá có ký tự ngoài BMP bị cắt sai giới hạn. Với dữ liệu Hán/Việt hiện có hai cách đếm trùng nhau, nên không kỳ vọng thấy khác biệt — nhưng nếu ai đó nhập khoá có emoji hoặc chữ ngoài BMP thì hành vi **đổi** (theo hướng đúng).
* **Grid bộ sưu tập mất swipe-to-delete/rename.** Hai việc đó chỉ còn ở `.contextMenu` (nhấn giữ) và ở menu trong `CollectionDetailView`. Người dùng quen vuốt sẽ không tìm thấy — đây là đánh đổi có chủ ý để đổi sang grid, không phải bỏ sót.
* **`LazyVGrid` mới của tab Bộ sưu tập nằm trong `ScrollView`, không trong `List`.** Giữ đúng bài học 1.3.269 (5 file `.ips` giống nhau, `EXC_BREAKPOINT` trong `CA::Transaction::commit`): lazy container trong một hàng `List` làm layout tự vô hiệu giữa lượt cập nhật cell. Cùng lý do, dải nút token dùng `FlowLayout` (custom `Layout`, **không** lazy) chứ không `LazyHStack`.
* **Chưa biên dịch**: viết trên Windows, không có `xcodebuild` lẫn `xcodegen`. Lượt này **thêm 7 file Swift** nên máy macOS phải `xcodegen generate` trước khi build. `check_architecture.py` giữ đúng **7 violation** (tập y hệt), `validate_links.py` PASS. Không dùng `Tests/` làm bằng chứng. CI xanh chỉ chứng minh *biên dịch được* — phần "đã bớt nóng" **phải** đo trên máy thật.
