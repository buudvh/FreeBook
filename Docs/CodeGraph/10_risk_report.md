...
* **A/B test (1.3.187, temporary):** the original combo `setCategory(.playback, mode: .spokenAudio, options: [.duckOthers, .allowBluetooth, .allowBluetoothA2DP])` was temporarily restored, and the `do/catch` was split so each call logs which one throws. This tests the hypothesis that removing `.allowBluetooth` (in 1.3.180's `efbe555`) caused the regression; the observed `-50` at HEAD occurred without `.allowBluetooth`, and Apple documents `.allowBluetooth` as valid only for input-capable categories (`playAndRecord`/`record`). Diagnostic logs are marked `REMOVE_AFTER_NOWPLAYING_DIAGNOSIS`.
* **A/B result (1.3.188):** device log confirmed `setCategory` still throws `OSStatus -50` even WITH `.allowBluetooth` — hypothesis refuted. Yet the lock screen / Control Center card NOW appears despite the `-50`, proving the session config was never the card blocker. The card appearance correlates with the 1.3.186 synchronous fallback publish. Session config reverted to the valid `.playback + .default + [.duckOthers]` combo (no `-50`); final verification requires the card to still appear without any `-50`.
