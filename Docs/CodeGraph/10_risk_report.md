...
* **Mitigated - hidden overlay work:** chapter-list queries and eager full-list title translation no longer run throughout ordinary reading and TTS updates.
...
*   **Vị trí**: [TTSBackgroundProcessor.swift](../../Sources/Services/TTS/TTSBackgroundProcessor.swift#L13), [TTSManager.swift](../../Sources/Services/TTS/TTSManager.swift#L525)
...
*   **Nguyên nhân**:
    ...
    *   Người dùng bấm chuyển chương nhanh liên tiếp có thể tạo ra nhiều Task xử lý nền chạy song song.
    *   Tác vụ cũ hoàn thành trễ hơn có thể đè đè dữ liệu mới nếu không được kiểm tra.
*   **Giải pháp (Mitigation)**:
    *   Mỗi khi bắt đầu phiên hoặc có yêu cầu chuyển chương mới, hệ thống cập nhật `sessionID` (UUID) và tăng `ttsProcessingGeneration` (Int) trên MainActor.
    *   Khi Actor xử lý nền hoàn tất và gọi callback MainActor, hệ thống kiểm tra nghiêm ngặt xem `sessionID` và `generation` nhận về từ DTO có khớp với giá trị hiện tại hay không; nếu không khớp (stale), kết quả sẽ bị hủy bỏ ngay lập tức.
