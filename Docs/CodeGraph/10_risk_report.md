...
* **Rule `<pn>` đi theo công tắc `isTranslationPronounsEnabled` (mặc định TẮT)** — hệ quả trực tiếp: bộ `Rule_new.txt` (16.941 rule `<pn>`) nhập được nhưng **không chạy** với thiết lập mặc định. Màn quản lý báo việc này bằng `DICT_TOKEN_WITHOUT_DICTIONARY` (mức `disabling`, không chặn nạp file).
