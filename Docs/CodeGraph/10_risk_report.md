...
* **LRU cap 3 của `QuickTranslationRuleBookStore` đổi `rowID` của bộ riêng khi bị đẩy ra rồi compile lại** (lần đọc sau không còn `previousSnapshot` để carry-forward UUID). Hệ quả thao tác giữa chừng: một `QuickTranslationRuleTrace.rowID` chụp trước đó nếu đem dùng sau khi truyện bị đẩy khỏi LRU (mở ≥ 3 truyện khác giữa chừng) sẽ bị store từ chối — "Rule đã không còn trong danh sách hiện tại" — thay vì xoá nhầm. Hướng an toàn đúng, nhưng người dùng phải chẩn đoán lại rồi thao tác lần hai; chỉ màn Check rule dính vì danh sách chính đọc snapshot mới mỗi lượt.
