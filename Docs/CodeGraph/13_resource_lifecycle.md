...
<!-- GENERATED START -->
## Reader resource lifecycle update (1.3.10)

`PrefetchManager` treats cancellation as a request, not completion: an active task retains its local and shared `ReaderPrefetchGate` concurrency slots until its fetch closure returns. This matters for synchronous JS/HTTP extension paths that cannot observe Swift task cancellation immediately, including work surviving a dismissed Reader while another Reader opens. Reader teardown cancels all reader-owned tasks; TTS resources continue under `TTSManager` ownership.

Speculative chapter loading has its own debounce lifecycle: it starts only after the center chapter is loaded, requests one forward neighbor, and is canceled by any subsequent jump, book change, or shutdown.

> Cap nhat 1.3.8: Reader window moi chi giu bounded chapter window trong mot `ScrollView`; cac chapter ngoai window duoc dua vao release queue cua `ChapterCache`. `TTSSession` giu playback snapshot doc lap voi `ReaderView`, trong khi `TTSManager` tiep tuc quan ly `AVAudioEngine` va cache PCM gioi han N/N+1.
