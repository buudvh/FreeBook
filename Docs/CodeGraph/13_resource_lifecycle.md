...
* **`FileHandle` của bản xuất TXT giờ sống suốt một tác vụ, và chủ đóng nó là ba đường tường minh.** `TxtExportFileWriter.init` tạo `<tên>.txt.part` rồi mở `FileHandle(forWritingTo:)` giữ cả phiên ([TxtExportFileWriter.swift:45](../../Sources/Services/Download/TxtExportFileWriter.swift#L45)); `finish()` đóng handle → xoá `.txt` cũ → `moveItem` sang tên thật ([#L69](../../Sources/Services/Download/TxtExportFileWriter.swift#L69)), `discard()` đóng handle → xoá `.part` ([#L80](../../Sources/Services/Download/TxtExportFileWriter.swift#L80)), và `deinit` chỉ `try? handle.close()` — **không** rename, **không** xoá, vì quyết định thuộc về hai hàm trên. `closeHandleIfNeeded()` có cờ `isClosed` nên đóng hai lần là vô hại, và `append` sau khi đóng bị `guard` bỏ qua thay vì throw.
* **Bất biến rác của `Documents/Exports/`**: trường hợp xấu nhất khi bị kill giữa tác vụ là còn một `.part`, không bao giờ là một `.txt` dở dang mà người dùng tưởng hợp lệ. Lượt xuất kế tiếp cùng tên tự `removeItem` `.part` cũ trước khi tạo mới ([#L35](../../Sources/Services/Download/TxtExportFileWriter.swift#L35)) nên phần sót không bị cộng vào bản mới. Đổi lại `String` cộng dồn cũ: đỉnh RAM không còn bằng cả file, chỉ còn bằng một chương.
