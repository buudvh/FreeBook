...
*   **Vòng đời (Lifecycle)**: `TTSManager.shared` là Singleton tồn tại suốt vòng đời app. Bắt đầu phát -> cấu hình AudioSession -> chạy AudioEngine và đồng bộ trạng thái khả dụng Remote Command Center (`playCommand`/`pauseCommand`) với `isPlaying` (`syncRemoteCommandState()`) -> phát gối đầu (prefetch WAV) -> dừng giải phóng tài nguyên. Tự động tìm kiếm cache và tải chương kế tiếp qua `advanceToNextChapter(nextIdx:)` khi hết chương.
*   **Rủi ro đã biết (Known Risks)**:
