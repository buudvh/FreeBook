...
*   **Vòng đời (Lifecycle)**: `TTSManager.shared` là Singleton tồn tại suốt vòng đời app. Bắt đầu phát -> cấu hình AudioSession -> chạy AudioEngine và đồng bộ trạng thái khả dụng Remote Command Center (`playCommand`/`pauseCommand`) giữ `isEnabled = active` cho cả 2 lệnh khi session active (thử nghiệm Phase D) để tránh hiện tượng nút Play bị mờ/xám sau khi pause, giữ `togglePlayPauseCommand.isEnabled = false` (thử nghiệm Phase C) để tránh xung đột nút điều khiển trung tâm trên Lock Screen, hỗ trợ bounded compatibility fallback cho lệnh remote `.pause` khi paused, đồng thời thực thi đồng bộ remote transport commands qua `dispatchRemoteTransportCommand` (dùng `MainActor.assumeIsolated` khi `Thread.isMainThread == true`) trước khi trả về status cho MediaRemote -> phát gối đầu (prefetch WAV) -> dừng giải phóng tài nguyên. Tự động tìm kiếm cache và tải chương kế tiếp qua `advanceToNextChapter(nextIdx:)` khi hết chương.
*   **Rủi ro đã biết (Known Risks)**:
