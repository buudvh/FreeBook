...
*   **Mục tiêu (Purpose)**: Hiển thị nội dung chương truyện sử dụng công nghệ vẽ chữ đồ họa CoreText, quản lý vị trí đọc hiện tại qua UICollectionView, tải trước (prefetch) chương kế tiếp ở mốc 70% và lưu trữ tiến trình đọc của người dùng.
*   **API công khai (Public API)**:
    ...
    *   `CoreTextHTMLParser.shared.parse(html:font:textColor:...) -> NSAttributedString`
    *   `CoreTextPaginator.shared.paginate(attributedString:bounds:chapterIndex:) -> [CoreTextPage]`
*   **Dependencies**: SwiftData (`ModelContext`), `ExtensionManager`, `ReadingProgressRepository`, `ChapterCache`, `PrefetchManager`, `CoreText`.
*   **Owned Objects**:
    *   `ReaderViewModel` sở hữu `ChapterCache` và `PrefetchManager`.
    *   `CoreTextScrollHostView` bọc `CoreTextCollectionScrollViewController` để quản lý `UICollectionView`.
*   **Điểm bắt đầu (Entry Points)**: `ReaderView.swift` khởi tạo `CoreTextScrollHostView` từ `BookDetailView.swift` khi người dùng nhấn nút "Đọc truyện".
*   **Điểm kết thúc (Exit Points)**: Trình đọc đóng, quay trở lại màn hình chi tiết sách.
*   **Vòng đời (Lifecycle)**: Khởi tạo -> Tải dữ liệu thô -> Chuyển sang HTML chứa div metadata -> Parse thành `NSAttributedString` kèm Custom Attributes (`.paragraphId`, `.originalText`, `.transText`) -> Phân trang có chiều cao cố định bằng `CoreTextPaginator` -> Vẽ chữ trực tiếp bằng `CTFrameDraw` lên các Cell của `UICollectionView` -> Cập nhật tiến trình qua callback `onProgressCommit`.
*   **Rủi ro đã biết (Known Risks)**:
    *   *Đã khắc phục*: Loại bỏ hoàn toàn lỗi đơ lag và lệch điểm cuộn khi chuyển chương xa nhờ việc sử dụng cell có chiều cao cố định (`readerHeight`) trên `UICollectionView`, giúp tính toán tọa độ cuộn tức thì (0ms).
    *   Xung đột thay đổi font/size làm thay đổi toàn bộ phân trang: Được giải quyết bằng cách xóa cache phân trang và cuộn neo lại vị trí đọc cũ một cách bất đồng bộ khi re-paginate.

---

## 2. Phân hệ Giọng đọc (TTS Subsystem)
*   **Mục tiêu (Purpose)**: Chuyển văn bản thành giọng nói (Text-To-Speech) bằng nhiều công cụ (Siri hệ thống, NghiTTS Piper offline, Extension JS), hỗ trợ highlight chữ đang đọc trên CoreText, và tự động chuyển tiếp/tải chương mới chạy nền khi phát đạt mốc 70% số đoạn.
*   **API công khai (Public API)**:
...
*   **Dependencies**: `AVFoundation`, `MediaPlayer`, SwiftData (`ModelContainer`), `SiriTTSService`, `ExtTTSService`, `PiperTTSService`.
*   **Owned Objects**:
    *   `TTSManager` sở hữu các TTS Engines (`SiriTTSService`, `ExtTTSService`, `PiperTTSService`, `NghiTTSClient`).
...
*   **Vòng đời (Lifecycle)**: Singleton `TTSManager.shared`. Bắt đầu phát -> cấu hình AudioSession -> chạy AudioEngine -> Bắn notification `ttsDidUpdateParagraphPosition` báo cho UIKit CollectionView cập nhật highlight/cuộn bám theo -> tự động prefetch chương kế tiếp khi đạt mốc 70% -> dừng giải phóng tài nguyên.
*   **Rủi ro đã biết (Known Risks)**:
    *   Xung đột đa luồng khi `completionHandler` của player node chạy trên thread riêng của AudioEngine, sau đó gọi lại `nextParagraph()` trên MainActor.
    *   Hỗ trợ tương tác "Nghe tại đây": Lắng nghe thông báo `ttsRequestPlayFromPosition` từ menu bôi đen chữ của UIKit để lập tức bắt đầu phát từ vị trí đã chọn.
