...
*   **Vòng đời (Lifecycle)**: `TTSManager.shared` là Singleton tồn tại suốt vòng đời app. Bắt đầu phát -> cấu hình AudioSession -> chạy AudioEngine và đồng bộ trạng thái khả dụng Remote Command Center (`playCommand`/`pauseCommand`) với `isPlaying` (`syncRemoteCommandState()`), hỗ trợ bounded compatibility fallback cho lệnh remote `.pause` khi paused, đồng thời thực hiện thử nghiệm kiểm chứng trên thiết bị thực tế chuẩn hóa metadata Now Playing (loại bỏ cờ `IsLiveStream` cũ, thiết lập `MPNowPlayingInfoPropertyMediaType.audio` và thống nhất thứ tự ghi `nowPlayingInfo` trước `playbackState`) -> phát gối đầu (prefetch WAV) -> dừng giải phóng tài nguyên. Tự động tìm kiếm cache và tải chương kế tiếp qua `advanceToNextChapter(nextIdx:)` khi hết chương.
*   **Rủi ro đã biết (Known Risks)**:
