...
## Tích hợp Anthropic Claude Messages API Native và Cập nhật Model Mới (1.3.406)

* **Tích Hợp Anthropic Claude Messages API (`AnthropicClient.swift`, `AnthropicTypes.swift`)**:
  - `AnthropicClient`: Triển khai singleton actor kết nối API native `/v1/messages` của Anthropic. Sử dụng header xác thực `x-api-key: <token>` và `anthropic-version: 2023-06-01`.
  - Tự động bóc tách tin nhắn `role == "system"` thành trường `system` cấp cao nhất của request JSON theo chuẩn Anthropic Messages API.
  - Chuẩn hóa luân phiên vai trò `user`/`assistant` và gộp các tin nhắn liên tiếp cùng vai trò, chèn placeholder nếu tin nhắn đầu tiên là assistant.
  - Hỗ trợ cả streaming Server-Sent Events (SSE) bắt sự kiện `content_block_delta` (`delta.text`) lẫn non-streaming.
  - Hàm `fetchAvailableModels` gọi `GET /models` với fallback thông minh về danh sách model chính thức mới nhất nếu endpoint không hỗ trợ hoặc bị chặn.
* **Cập Nhật Danh Sách Model Chính Thức và Định Dạng API Profile (`AIProviderProfile.swift`, `AIProviderPreset.swift`)**:
  - Bổ sung trường `apiFormat: String` ("openai" hoặc "anthropic") vào `AIProviderProfile`, hỗ trợ giải mã tương thích ngược.
  - Thêm preset `defaultAnthropic` với các model chính thức: `claude-3-7-sonnet-latest`, `claude-3-5-sonnet-latest`, `claude-3-5-haiku-latest`, `claude-3-7-sonnet-20250219`, `claude-3-5-sonnet-20241022`, `claude-3-opus-latest`.
...
* **Rẽ Nhánh Điều Phối AI Client Trong Toàn Bộ Ứng Dụng**:
  - `AIRuntimeCoordinator.swift`: Rẽ nhánh streaming gọi `AnthropicClient.shared.sendChatStreaming` khi `activeProfile.apiFormat == "anthropic"`.
  - `AIContextCompactor.swift` & `AINameExtractionBatchProcessor.swift`: Rẽ nhánh gọi `AnthropicClient.shared.sendChat`.
  - `AISettingsView.swift`, `AISettingsView+Actions.swift`, `AddProviderProfileSheet.swift`: Bổ sung Picker chọn định dạng API, template Anthropic Claude chính thức, nút tải model từ API và test kết nối native.
...
* **Điều Phối Vòng Đời Tác Vụ Ngầm & Toast Hoàn Thành (`AIRuntimeCoordinator.swift`)**:
  - `AIRuntimeCoordinator`: Quản lý `Task` chạy ngầm độc lập khỏi vòng đời của View cho streaming chat, lọc name chương hiện tại và quét batch các chương đã tải.
...
* **Đồng Bộ Chỉ Báo "AI đang suy nghĩ" & Đóng An Toàn Khi TTS Điều Hướng (`ReaderAIFullScreenView.swift`, `ReaderAIFullScreenView+Actions.swift`, `ReaderView.swift`, `ReaderView+AI.swift`)**:
  ...
  - Chuyển `onOpenAI` trong `ReaderView` sang `openAIFromReader()`, loại bỏ modal `.fullScreenCover` cục bộ giúp giảm dòng code trong `ReaderView.swift`.
...
* **Panel Dịch của Reader**: chẩn đoán rule chạy off-main + debounce. Chip rule nay đi theo `originalSentence` và signal rule riêng, không chạy lại theo từng lần nới/thu vùng chọn.
...
* **Phân hệ gợi ý phiên âm tách khỏi màn từ điển TTS.** [`AddWordSheet`](../../Sources/Views/Settings/TTS/AddWordSheet.swift) là file riêng và là **chủ sở hữu duy nhất** của trạng thái gợi ý (`suggestions`, `isBuildingSuggestions`, `suggestionLoadTask`). `TTSPhoneticSuggestionBuilder` vẫn là bộ dựng duy nhất; điều đổi là nó được gọi từ `Task.detached` chứ không từ `body` — trước đây mỗi lượt vẽ là một lượt gọi espeak đồng bộ trên main thread.
* **Màn tìm trong Kệ sách & Lịch sử nay là *chỗ thứ ba* dùng chung bộ hành động truyện**, sau tab Kệ sách và màn bộ sưu tập: cùng `BookActionSheet`, cùng `BookActionRunner`, cùng bộ sheet/navigation phụ. Chế độ sheet suy ra từ dữ liệu (`book.isOnShelf ? .shelf : .history`) chứ không cố định theo màn — vì danh sách kết quả trộn cả truyện trên kệ và truyện chỉ còn trong lịch sử. Phần hành động nằm ở [`ShelfSearchView+Actions`](../../Sources/Views/Shelf/ShelfMain/Extensions/ShelfSearchView+Actions.swift).
* **Quản lý chính bộ sưu tập giờ có hai lối vào, không còn một.** [`CollectionDetailView+Manage`](../../Sources/Views/Shelf/Collections/CollectionDetailView+Manage.swift) thêm menu `ellipsis.circle` (Đổi tên / Xoá) ngay trong màn chi tiết bộ, và `CollectionsTabView` thêm `.contextMenu` cho hàng bộ. Swipe action cũ giữ nguyên. Chủ transaction vẫn **chỉ** là `BookCollectionCoordinator`.
* **Panel nhập rule: vế phải lên ngang vế trái.** Ô Bản dịch dùng chính `QuickTranslationRulePatternField` (khác đúng một tham số `usesMonospacedFont`), nên chip `{i}` chèn tại con trỏ. `QuickTranslationRuleCaptureChipsView` không đổi — nó vẫn chỉ phát chỉ số, phía sheet quyết định chèn vào đâu.
* **Trung tâm thông báo dịch tên chương tại chỗ hiển thị**, không đụng `NewChapterProbe`/`NewChapterRecord`: `latestChapterTitle` vẫn lưu nguyên văn từ nguồn, `NotificationInboxView.displayedChapterTitle` mới là chỗ gọi `TranslateUtils.translateChapterTitle`.
...
* **Thành viên mới của phân hệ: `ExtTTSScriptCache`**, và nó là **chủ sở hữu duy nhất** của việc đọc `tts.js` + trộn config cho TTS. Hai lối vào (`ttsGenerate` để chạy, `getTTSRuntimeFingerprint` để dựng `synthesisKey`) giờ chia nhau một `Payload`, thay vì mỗi bên tự đọc đĩa như trước.
* **`ExtTTSRuntime` giữ nguyên ngữ nghĩa, đổi khoá so sánh**: danh tính runtime là `(localPath, downloadUrl, fingerprint)`. Fingerprint đã băm script + config + đường dẫn nên không mất độ phân giải, mà bỏ được phép so chuỗi vài chục KB mỗi đoạn văn.
* **Phân hệ Tiện Ích (tab Tiện Ích) nay có chính sách chạy riêng**, đúng khuôn phân hệ Chương mới: `RepositoryRefreshPolicy` (cooldown 6 giờ, `UserDefaults`) cho lượt **tự động** lúc mở tab; nút refresh trên toolbar bỏ qua cửa; `markRefreshed()` chỉ ghi khi ≥ 1 kho cập nhật được. Trước lượt này `.onAppear` quét mạng vô điều kiện.
* **Ranh giới trình bày của tab được dọn**: `filteredExtensions` tính một lần rồi truyền vào `filterStatusBar(count:)`/`isEmpty`/`List`, và icon hàng dùng `ExtensionIconView` (cục bộ trước, mạng sau) thay cho `AsyncImage` — tiện ích đã cài không còn tải icon qua mạng.
* **`ExtensionIconView` có cache dùng chung cho cả app**: `ExtensionIconImageCache`. Đây là thay đổi lan ra ngoài phân hệ Tiện Ích — view này nằm trong mọi dòng Kệ sách, Khám phá và mục lục Reader, và trước đây mỗi lượt vẽ là một lần đọc đĩa + giải mã PNG.

## Hộp thư chương mới: thông báo tách khỏi badge (1.3.329)

* **Trước lượt này "có chương mới" và "dòng trong Trung tâm thông báo" là **một** con số, nên đánh dấu đã đọc làm mất luôn thông báo.** `NewChapterRecord.markSeen()` đưa `newChapterCount` về 0 và `firstFoundAt` về `nil`, còn `NotificationInboxView` lọc dòng bằng `hasNew` — bấm vào dòng là dòng biến mất. Tệ hơn: `NewChapterProbe.applyDiff` **luôn** ghi lại `newChapterCount` mỗi lượt dò, nên chỉ giữ con số cũ cũng không đủ.
* **Cách tách**: `NewChapterRecord` có thêm nhóm `announced*` (`announcedChapterCount`, `announcedIsCountExact`, `announcedAt`, `announcementReadAt`). Hai nhóm có hai chủ và hai vòng đời:
  * `newChapterCount`/`isCountExact`/`firstFoundAt` = **trạng thái badge**, chủ là `NewChapterProbe`, bị ghi lại mỗi lượt dò, về 0 khi `markSeen()`.
  * `announced*` = **thông báo đã phát**, chủ là `NewChapterRecord.announceCurrentFinding()` (probe gọi khi tìm ra chương mới), chỉ mất khi người dùng xoá dòng.
* **Luật một đợt thông báo**: đợt mới (chưa có thông báo, hoặc thông báo trước đã đọc) bắt đầu một dòng **chưa đọc** với mốc `firstFoundAt`; đợt đang chờ đọc mà dò thêm được thì chỉ **cập nhật con số**, giữ nguyên mốc phát hiện đầu tiên. Nhờ vậy đọc rồi mà có thêm chương thì lại nổi lên như thông báo mới, còn kiểm tra ba lượt trong một đợt không sinh ba dòng.
* **Badge không đổi hành vi một chút nào**: `hasNew`, `totalNewBooks`, `NewChapterBadgeView` vẫn đọc `newChapterCount` nên badge kệ sách và badge chuông vẫn tắt ngay khi đánh dấu đã đọc. Đây là lý do chọn cách thêm field thay vì đổi nghĩa `hasNew` — không phải sửa một call site nào ở tầng View.
* **Trung tâm thông báo nay đối xử hai loại nội dung như nhau**: dòng chương mới có dấu chấm chưa đọc, đổi icon `bell.badge.fill` → `bell` khi đã đọc, swipe để xoá, và "Xoá thông báo đã đọc" dọn **cả hai** loại. Xoá một dòng chỉ gọi `clearAnnouncement` — **không** xoá cả record, vì record giữ mốc `seen*`; xoá nó là lượt dò sau báo lại từ đầu.
* **Có bước vá dữ liệu cũ**: `loadIfNeeded()` gọi `backfillAnnouncements()` để record của bản app trước (chỉ có `newChapterCount`) sinh ra thông báo tương ứng — nếu không, người vừa cập nhật sẽ thấy badge sáng mà Trung tâm thông báo trống. Đây là chỗ duy nhất `new_chapters.json` được "migrate", và nó chạy được vì `NewChapterRecord.init(from:)` vốn `decodeIfPresent` mọi khoá.

## Quản lý rule dịch: ẩn nút tải bộ mặc định khi máy đã có rule (1.3.329)

* Nút "Tải bộ rule mặc định" chỉ hiện khi `store.status.ruleCount == 0`. Nó đi qua đúng đường `importRules` với chế độ **ghi đè** bộ đang chạy, nên với người đã sửa/nhập bộ riêng thì đó là một cú mất dữ liệu cách một lần bấm — và màn này không có undo.
* **Không tạo đường cụt**: "Xoá bộ rule khỏi máy" đưa `ruleCount` về 0 và nút hiện lại; "Nhập file rule (.txt)" vẫn luôn có. Footer của section đổi theo trạng thái để nói rõ vì sao nút biến mất.
* Điều kiện là `ruleCount == 0` chứ **không** phải `!isLoaded`: `makeStatus` đặt `isLoaded = true` cho mọi snapshot, kể cả file phân tích ra 0 rule — đúng trường hợp người dùng cần tải lại bộ mặc định nhất.

## Phân hệ mới: Bộ sưu tập sách; Kệ sách đổi cách nhấn giữ (1.3.328)

* **Phân hệ Bộ sưu tập là một lát dọc đầy đủ chứ không phải một màn hình**: `BookCollection` (`@Model`) → `BookCollectionCoordinator` (chủ transaction) → `CollectionsTabView` / `CollectionDetailView` / `CollectionPickerSheet` / phần bộ sưu tập trong `BookActionSheet`, cộng đường sao lưu `library/collections.json`. Nó **không** mở cơ chế mới nào: cùng khuôn coordinator + `Result`, cùng luật View chỉ `@Query` để đọc.
* **Hai bất biến định nghĩa phân hệ** (và là chỗ dễ làm sai nhất):
  1. Truyện nằm trong bộ sưu tập thì **bắt buộc ở trên kệ** — mọi lối thêm đều bật `isOnShelf` (`BookCollectionCoordinator.promoteToShelf`).
  2. Bỏ khỏi bộ sưu tập / xoá bộ sưu tập **không bao giờ** làm truyện rời kệ hay bị xoá — `deleteRule: .nullify` ở cả hai đầu quan hệ N-N.
  Chiều còn lại được `BookTransactionCoordinator` giữ: rời kệ là dọn sạch `collections` và `isPinned`.
* **Kệ sách nay có 4 tab**: Downloads (0), Kệ Sách (1), **Bộ Sưu Tập (2)**, Lịch Sử (**3**). Số tab là hợp đồng liên màn — `SearchView` gửi `shelfTab` qua notification `sourceChangedNavigateToShelf`, nên đổi số ở `ShelfView` mà quên đầu kia là điều hướng sai tab, im lặng.
* **Nhấn giữ một cuốn sách không còn là `contextMenu`.** Menu ngữ cảnh của SwiftUI chỉ nhận `Button`/`Link`, không dựng được phần đầu (bìa + tên + tác giả) lẫn danh sách bộ sưu tập bấm thêm/bớt tại chỗ. Thay bằng `BookActionSheet`, và dòng truyện đổi từ `Button { } label: { }` sang `onTapGesture` + `onLongPressGesture` — bọc trong `Button` thì cú nhả tay sau khi giữ vẫn kích hoạt action, mở luôn cả Reader. **Toàn bộ mục cũ được giữ nguyên chữ và thứ tự**, thêm Ghim/Bỏ ghim và (khi mở từ trong một bộ) "Bỏ khỏi bộ sưu tập này".
* **Ba màn dùng chung một bộ hành động**: Kệ sách, Lịch sử và màn Bộ sưu tập cùng phát `BookSheetAction`, thân việc nằm ở `BookActionRunner`. Đây là lý do "truyện trong bộ sưu tập có mọi hành động như trên kệ" đúng theo định nghĩa, không phải nhờ copy hai bản rồi cố giữ cho giống nhau. Phần *trình bày* (navigation/sheet) vẫn thuộc từng màn vì mỗi màn có stack riêng.
* **Sheet đóng trước, hành động phát ở turn sau** (`BookActionSheet.emit`): phần lớn mục mở tiếp một sheet khác (`TaskOptionsSheet`, `BookInfoEditView`) hoặc một `navigationDestination`, và phát ngay trong lúc sheet này còn đang đóng thì hai lớp trình bày chọi nhau — lớp mới không hiện. Cùng lý do và cùng cách xử lý với `NotificationInboxView(onOpenBook:)` ở `ShelfView`. Ngoại lệ vẫn là phần bộ sưu tập: nó không mở gì nên ghi trực tiếp và sheet ở lại để người dùng thấy danh sách vừa đổi.
* **Ghim là thứ tự hiển thị, không phải nhóm dữ liệu**: cùng ngữ nghĩa `Extension.isPinned` của phân hệ Tiện ích, sắp xếp làm trên RAM (`@Query` vẫn sort theo `lastReadDate` vì tab Lịch sử dùng chung query đó).
* **Phân hệ Backup mở rộng mà không thêm `BackupScope`**: bộ sưu tập + cờ ghim đi kèm nhóm `.books` (nhóm bắt buộc). Thêm case `BackupScope` là làm bản app cũ decode `manifest.scopes` lỗi — luật này đã ghi hai lần trong `BackupPaths.swift` và vẫn được giữ. Khôi phục là **gộp**: bộ trùng tên dùng lại bộ đang có, chỉ bổ sung thành viên có mặt trong máy.

## Phân hệ tiền xử lý TTS mất thước đo trong app (1.3.328)

* **Màn "Thử phiên âm" bị xoá theo yêu cầu**, kéo theo `TransliterationGoldenSet`, `EspeakPhonemizer.probeVoices` và `VietnameseTokenGate.explain` (đều không còn caller). Hệ quả phải nói thẳng: phân hệ này **không còn cách kiểm chứng có hệ thống nào** — `Tests/` bị coi như không tồn tại, máy chạy qua LiveContainer không đính được debugger, nên từ nay việc xác minh bảng âm vị chỉ còn là **nghe thử** qua màn "Thử giọng đọc" (`NghiTTSTextToolView`). `rules.md` và `10_risk_report` đã được cập nhật theo.
* Công tắc `EnglishPhonemeTransliterator.useEspeakKey` mất chỗ ở cũ nên được **chuyển** sang `NghiTTSSettingsView` ("Dùng IPA của espeak cho tiếng Anh") — nếu bỏ luôn thì đường IPA của espeak thành bật vĩnh viễn, không có đường thoát khi nó đọc sai một loạt từ.

## Hai màn quản lý quy tắc gom hết nút vào một dropdown ở chỗ nút Edit (1.3.327)

* **Thanh công cụ đáy bị bỏ ở cả hai màn** ([JunkFilterManagementView](../../Sources/Views/Settings/Translation/JunkFilterManagementView.swift#L1), [TTSReplacementManagerView](../../Sources/Views/Settings/TTS/TTSReplacementManagerView.swift#L1)). Trước đây mỗi màn có ba điểm điều khiển: `EditButton()` ở `.navigationBarTrailing`, `Menu` "Tùy chọn" và nút `+` ở `.bottomBar`. Nay còn **một** `ToolbarItem(placement: .topBarTrailing)` — đúng chỗ `EditButton` cũ — chứa toàn bộ: Thêm quy tắc → Sắp xếp lại → Nhập/Xuất JSON → (lọc rác) Xoá tất cả / (TTS) Khôi phục mặc định.
* **`EditButton()` bị thay bằng `@State editMode` + `.environment(\.editMode, $editMode)`.** Lý do không phải là thẩm mỹ: `EditButton` tự quản trạng thái nên `Menu` không đọc được để đổi nhãn, và nhãn của nó do hệ thống dịch nên lệch với toàn bộ chữ tiếng Việt hardcode ở hai màn này. Mục trong menu vì vậy đổi giữa "Sắp xếp lại" ⇄ "Xong sắp xếp".
* **Bất biến cũ được giữ nguyên và nay có thêm một chốt**: mục sắp xếp vẫn **ẩn khi đang tìm kiếm** (thứ tự hiện ra không phải thứ tự áp dụng, `onMove` chỉ gắn ở nhánh không lọc), và `.onChange(of: isSearching)` đưa `editMode` về `.inactive` khi người dùng bắt đầu gõ tìm — nếu không List sẽ kẹt ở edit mode trong khi mục thoát duy nhất đã bị ẩn khỏi menu.
* Hành vi dữ liệu không đổi một dòng nào: `onDelete`/`onMove`/`swipeActions`, hai `confirmationDialog` nhập/khôi phục, `DocumentPickerPresenter` và `ShareSheet` giữ nguyên vị trí trong chuỗi modifier.

## Debug Extension: cài mới được extension chưa có trên app (1.3.325)

* **`draft.install` có hai nhánh, tự chọn theo trạng thái thư viện.** Đã có extension ⇒ `installOverExisting` (ghi đè file, metadata thư viện không đổi — hành vi cũ). Chưa có ⇒ `installAsNew`: `ExtensionDraftInstaller.installNew` dựng `extensions/<packageId>/` từ thư mục staging, rồi router ghi hàng `Extension` qua `ExtensionTransactionCoordinator`. Không thêm lệnh mới vào protocol: client vẫn gửi đúng `draft.install` với `packageId` + `sourceRevision`.
* **Danh tính do app quyết, đọc từ `plugin.json` của bản nháp** ([ExtensionDraftMetadata](../../Sources/Services/Extensions/Debug/Staging/ExtensionDraftMetadata.swift#L1)) theo **đúng** luật của `ExtensionManager.installFromLocalZip`: `metadata.packageId` thắng, thiếu thì `name.lowercased()` thay dấu cách bằng `_`. Router tra thư viện bằng **cả hai** id (id client gửi và id app suy ra), nên lệch id vẫn rơi vào nhánh *cập nhật* thay vì tạo hàng SwiftData thứ hai.
* **Ranh giới giữ nguyên: installer chỉ đụng file, router mới là chỗ ghi SwiftData.** `ExtensionDraftInstaller` là actor nền không giữ `ModelContext`; `writeLibraryRow` hop sang `MainActor`, dựng `ModelContext(container)` **riêng** cho lượt ghi rồi phát `"extensionDidUpdate"` — cùng khuôn với `BackupRestoreWorker` khi restore extension từ nền.
* **Thứ tự file → bản ghi là bất biến an toàn**, không phải chi tiết triển khai: bản ghi trỏ vào thư mục không tồn tại là lỗi im lặng ở mọi màn `@Query`, còn thư mục mồ côi thì `ExtensionInstallAudit` thấy được và lần cài sau tự sao lưu rồi thay.
* **`run.start` với `sourceMode: "draft"` chạy được extension chưa cài** — cách thử trước khi thêm vào thư viện. Thiếu bản đã cài thì `downloadUrl`/`configJson` rỗng và `host` lấy `metadata.source` của bản nháp; `getCombinedConfigs` vẫn nạp mặc định từ khoá `config` trong `plugin.json`.
* **`draft.stage` bỏ chốt "phải đã cài"**: chốt thật của vùng staging là trần `ExtensionDraftManifest` (200 file / 1 MiB mỗi file / 4 MiB tổng), kiểm tra containment từng path, và staging bị xoá sạch khi tắt server hoặc mở lại app. Ghi vào thư viện vẫn phải bấm trên thiết bị.
* **Cửa xác nhận nói rõ hơn**: `Kind.installNew` + `Request.displayName` (tên đọc từ `plugin.json`), màn Debug Server đổi nhãn "Đồng ý cài mới", header "Yêu cầu thêm extension mới" và footer cảnh báo app sẽ tạo thư mục + bản ghi mới. Client cũng nạp lại `extensions.list` sau khi cài để lần chạy sau ghép đúng id.

## Debug Extension: hết lệch packageId, Run Current File hết thiếu tham số (1.3.324)

* **Nguồn sự thật của `packageId` là `extensions.list`, không phải `plugin.json`.** `resolvePackageId()` (`Tools/VSCode/FreeBookExtDebug/src/extension.ts`) đổi id đoán từ thư mục sang id thật của app theo bốn bước: trùng khít → trùng slug của tên → slug tên trùng slug tên → tên rút gọn (bỏ dấu, bỏ ký tự không phải chữ/số) trùng nhau. Không khớp được thì client báo thẳng "app không có extension nào khớp" kèm danh sách id, thay vì để server trả `UNKNOWN_EXTENSION`. Mọi lệnh có `packageId` (`run.start`, `draft.stage`, `draft.install`, `draft.rollback`) **và** khoá của `stagedRevisions` đều đi qua hàm này, nên id dùng để stage và id dùng để install luôn là một.
* **`slugPackageId`/`compactName` ở `draft.ts` là phỏng đoán có chú thích, không phải hợp đồng.** App vẫn là thẩm quyền cuối cùng về danh tính; hai hàm này chỉ để ghép cặp thư mục nguồn ↔ extension đã cài.
* **"Run Current File" không còn gửi run trắng.** Trước đây nó lấy tên file làm entrypoint rồi gửi **không tham số**, nên `search`/`detail`/`toc`/`chap` luôn bị `UNKNOWN_ENTRYPOINT` (`entrypoint(from:)` trả `nil` khi thiếu `keyword`/`url`) và chỉ `genre`/`home` chạy được. Nay `collectEntrypointParams` hỏi `keyword`/`url` trước khi gửi và dùng chung với "Run Script"/"Run Profile"; Esc ở ô bắt buộc là dừng, không gửi lệnh.
* **Điều kiện chọn đường `custom` đổi từ "không có trong `script` của `plugin.json`" sang "không thuộc sáu entrypoint chuẩn".** Khoá trong `script` là tên script của extension, không phải entrypoint của server — `list` khai trong `script` vẫn phải đi đường `custom` kèm `scriptFileName`.
* **Stage khi chọn extension từ danh sách app không còn gói nhầm thư mục**: `getActiveFolderUri()` tìm thư mục workspace cùng danh tính trước khi rơi về `workspaceFolders[0]` (mặc định cũ chỉ đúng khi workspace đúng là thư mục extension).
* **Giới hạn chưa đổi, và là chủ ý chứ không phải bug**: `draft.stage` / `run.start` / `draft.install` vẫn **bắt buộc** extension đã có trên app. `ExtensionDraftInstaller` chỉ thay file trong `snapshot.localPath` và cố ý không ghi SwiftData, nên **chưa có** đường nào cài một extension hoàn toàn mới từ VS Code.
