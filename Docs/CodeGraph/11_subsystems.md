...
*   **Vòng đời (Lifecycle)**: `TTSManager.shared` là Singleton tồn tại suốt vòng đời app. Bắt đầu phát -> cấu hình AudioSession -> chạy AudioEngine và đồng bộ trạng thái khả dụng Remote Command Center (`syncRemoteCommandState()`), bật `togglePlayPauseCommand.isEnabled = active` và đặt `playCommand.isEnabled = false`, `pauseCommand.isEnabled = false` (thử nghiệm kiểm soát Phase E) nhằm khắc phục việc gửi nhầm `action:pause` khi tạm dừng, giữ bounded compatibility fallback cho lệnh remote `.pause` khi paused, đồng thời thực thi đồng bộ remote transport commands qua `dispatchRemoteTransportCommand` (dùng `MainActor.assumeIsolated` khi `Thread.isMainThread == true`) trước khi trả về status cho MediaRemote -> phát gối đầu (prefetch WAV) -> dừng giải phóng tài nguyên. Tự động tìm kiếm cache và tải chương kế tiếp qua `advanceToNextChapter(nextIdx:)` khi hết chương.
*   **Rủi ro đã biết (Known Risks)**:
