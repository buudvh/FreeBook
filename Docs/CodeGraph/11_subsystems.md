...
<!-- GENERATED START -->
## Reader subsystem update (1.3.10)

The Reader subsystem keeps a bounded render window synchronized through `ReaderViewModel.updateActiveLocationFromScroll(chapterIndex:paragraphIndex:persistProgress:)`. `persistProgress` separates visual/manual navigation from TTS-owned progress, and `shutdown(saveProgress:) async` defines teardown for reader-owned prefetch, debounce, cache, and conditional final persistence resources.

`ReaderViewModel` uses target-first loading for window replacement. `scheduleSettledPrefetch` waits for the selected chapter to reach `.loaded` before enqueueing one forward neighbor; replacing the window cancels that speculative work.

> Cap nhat 1.3.8: Reader runtime da co mot `ScrollView` doc duy nhat voi `LazyVStack` cho bounded chapter window; Jump To Chapter la thao tac `replaceWindow(center:)` thay vi scroll qua toan bo truyen. TTS co `TTSSession` snapshot + `PlaybackQueue` de Reader co the dong/mo lai ma khong so huu playback.
...
*   **Dependencies**: `AVFoundation`, `MediaPlayer` (Now Playing Info & Remote Command Center), SwiftData (`ModelContainer`), `SiriTTSService`, `ExtTTSService`, `PiperTTSService`, `ExtensionManager`, `Chapter`.
*   **Owned Objects**:
