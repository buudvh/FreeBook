...
<!-- GENERATED START -->
## Reader subsystem update (1.3.11, supersedes 1.3.10)

The Reader runtime renders one chapter in one vertical `ScrollView`. Chapter changes come only from footer buttons, horizontal swipe, chapter list, history restore, or TTS sync; reaching the vertical end has no navigation side effect.
...
The Reader chrome uses a fixed three-row header: navigation commands occupy the first row, while one translation control spans the book-title and chapter-title rows. The chapter-title row opens the always-mounted table of contents, which slides vertically from the bottom. Its metadata header reuses existing book title, author, and cover data without reloading book details; the body exposes TTS as one floating control.
