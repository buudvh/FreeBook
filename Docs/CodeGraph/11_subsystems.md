...
<!-- GENERATED START -->
## Sao lưu/Khôi phục: tiến độ tải xuống thật và khoá đăng xuất (1.3.248)

* **Tiến độ tải xuống từ Drive nay là số thật, không còn thanh giả.** Trước đây `GoogleDriveClient.download(file:)` không báo byte nào, nên `BackupProgress.totalUnits` giữ 0 ⇒ `fraction` trả `nil` ⇒ `ProgressView(value: nil)` kiểu `.linear` hiện một thanh **tĩnh** suốt cả lượt tải. Nay `download(file:report:)` nhận closure báo tiến độ và gắn một **task delegate** (`DownloadProgressObserver`) vào `URLSession.shared.download(for:delegate:)`. Đây là đường duy nhất khả dụng: bản async của `download` không trả về `URLSessionTask` nên không có handle để đọc `progress`, còn duyệt từng byte kiểu `for await` thì quá chậm với archive vài trăm MB.
* Ba bất biến của observer, mỗi cái vá đúng một cái bẫy: (1) chỉ cài `didWriteData` — cài `didFinishDownloadingTo` là tranh chấp với chính bản async vốn tự dời file tạm, nên hàm đó cố ý để **trống** (protocol yêu cầu); (2) `totalBytesExpectedToWrite <= 0` (Drive có thể không gửi `Content-Length`) thì dùng `GoogleDriveFile.byteCount` lấy từ lượt `listBackups`; (3) đơn vị báo ra là **phần trăm** (`completedUnits: percent, totalUnits: 100`) và chỉ báo khi phần trăm **đổi** — delegate được gọi mỗi lần có gói dữ liệu về, không tiết chế thì hàng nghìn lượt hop lên MainActor. Observer được giữ **mạnh** trong thân `download` vì `download(for:delegate:)` chỉ giữ delegate yếu.
