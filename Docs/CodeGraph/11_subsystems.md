...
*   **Owned Objects**:
    ...
    *   `NghiSynthesisPolicy` là policy tĩnh điều phối watermark/cooldown theo nhiệt; `PiperSynthesisCoordinator` vẫn sở hữu slot tổng hợp NghiTTS duy nhất.
...
*   **Vòng đời (Lifecycle)**: `TTSManager.shared` là Singleton tồn tại suốt vòng đời app. Bắt đầu phát -> cấu hình AudioSession -> chạy AudioEngine và đồng bộ trạng thái khả dụng Remote Command Center (`syncRemoteCommandState()`), cập nhật khả dụng phụ thuộc trạng thái phát cho `playCommand` (`paused`) và `pauseCommand` (`playing`) trong khi duy trì `togglePlayPauseCommand` (`active`, Phase J), giữ bounded compatibility fallback cho lệnh remote `.pause` khi paused, đồng thời thực thi đồng bộ remote transport commands qua `dispatchRemoteTransportCommand` (dùng `MainActor.assumeIsolated` khi `Thread.isMainThread == true`) tính toán status phản hồi (Phase F) và đồng bộ xuất bản metadata hợp đồng timeline hữu hạn `currentRate: 1.0/0.0` / `defaultRate: speed` / `elapsed` / `duration` / `progress` (Phase I) qua `setSystemNowPlayingPlaybackState(_:)` ngay trước khi remote callback trả về (hàm `updateNowPlayingInfo()` hoàn tất bất đồng bộ sau đó nhưng bảo tồn cùng quy chuẩn tốc độ và timeline, không làm đè dữ liệu cũ) -> phát gối đầu (prefetch WAV) -> dừng giải phóng tài nguyên. Tự động tìm kiếm cache và tải chương kế tiếp qua `advanceToNextChapter(nextIdx:)` khi hết chương.
*   **Rủi ro đã biết (Known Risks)**:
