...
| [`Views/Settings/Translation/QuickTranslationRulesView.swift`](../../Sources/Views/Settings/Translation/QuickTranslationRulesView.swift) | màn quản lý: trạng thái, nút tải về, nhập (3 chế độ) / xuất / xoá, đường vào 2 công cụ | 299 |
...
| [`Views/Settings/Translation/QuickTranslationRuleListView.swift`](../../Sources/Views/Settings/Translation/QuickTranslationRuleListView.swift) | **màn riêng**: danh sách rule + `.searchable` (mẫu / bản dịch / số dòng), phân trang 200, badge cảnh báo, nút `+` và swipe Sửa/Xoá | 234 |
...
| [`Services/Translation/Engine/QuickTranslationRuleFileEditor.swift`](../../Sources/Services/Translation/Engine/QuickTranslationRuleFileEditor.swift) | phẫu thuật theo dòng, địa chỉ hoá theo key: `upsert` / `delete` / `update` / `merge` (3 chế độ) / `importPreview`, giữ comment và thứ tự | 194 |
| [`Services/Translation/Extensions/QuickTranslationRuleStore+Editing.swift`](../../Sources/Services/Translation/Extensions/QuickTranslationRuleStore+Editing.swift) | 3 hàm CRUD của store, đều đi qua `importRules` nên giữ validate-then-swap | 57 |
...
* Tổng file Swift 362 → **386** (+24, không xoá file nào). Không thư mục resource nào mới. `Sources/Services/Translation/Engine/` 1 → **16** file; `Sources/Services/Translation/Extensions/` 1 → **3** file; `Sources/Views/Settings/Translation/` 2 → **7** file; `Sources/Models/Dictionaries/` +1 (`DataImportMode`).
